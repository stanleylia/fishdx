...
OUT = RES / "mcnemar_d2_reproduced_faithful_O.json"
...
def main() -> None:
    ...
    for i in range(n):
        ...
        # ADR-0012e faithful evidence pool: 𝒪 = caption ∪ top-1 retrieved KB doc
        # text. top-1 doc = the KB document for the retrieved class c1.
        evidence = qcap[i] + "\n" + doc_text.get(c1, "")
        s_h = compute_s_h(
            evidence,
    ...
    out = {
        "experiment": "mcnemar_d2_reproduced_faithful_O",
        "reviewer_point": "R5.3 — actual 2x2 contingency tables from real paired D2 predictions",
        "note": (
            "FAITHFUL-𝒪 variant. Identical to mcnemar_d2_reproduced.json (full n=3,453, "
            "all 8 classes incl EUS, augmentations included, seed 42, cached captions) "
            "EXCEPT the Stage-3 evidence pool now follows ADR-0012e Option B: "
            "𝒪 = caption ∪ top-1 retrieved KB doc text (evidence = caption + '\\n' + "
            "full text of the KB document for the retrieved top-1 class c1). By "
            "construction c1's own doc keywords then match, so S_d for c1 confirms the "
            "retrieval top-1 class (the deployed-pipeline mechanism). The original "
            "manuscript discordant counts are recorded under 'original_manuscript'."
        ),
        "metadata": {
            "gpu": gpu,
            "device": DEVICE,
